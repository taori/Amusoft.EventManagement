...
{
	[TestFixture]
	public class AsyncEventHandlerOrdinaryTests : IntegrityTestBase<AsyncEventHandler, AsyncEventHandlerReceiver, AsyncEventHandlerOrdinarySource>
	{
		[Test]
		...
		public override async Task RaiseWithNoSubscribe()
		{
			var receiver = CreateEventReceiver();
			var source = CreateEventSource();
			var invoke = source.Invoker?.Invoke(this, EventArgs.Empty);
			if (invoke != null)
				await invoke;
		...
		public override async Task SubRaiseUnsubRaiseSubRaise()
		{
			...
			invoke = source.Invoker?.Invoke(this, EventArgs.Empty);
			if (invoke != null)
				await invoke;
			Assert.That(receiver.CallCount, Is.EqualTo(1));
